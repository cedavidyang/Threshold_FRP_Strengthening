...
class MainSmp(object):
    def __init__(self, nsmp=None, gmdistr=None, r_array=None, sl_array=None, gd_array=None, preSmp=None):
        """Create object:
        :Args:
        ...
        """
        if preSmp==None:
            if any((nsmp==None, gmdistr==None, r_array==None, sl_array==None, gd_array==None)):
...
#=============================================================================#
    ...
    def priorPDF(self, smps):
        ...
        for i in range(smps.shape[1]):
            pdfs[:,i] = self.r_array[i].pdf(smps[:,i])
    ...
    def condAvailability(self, smps, ti):
        ...
        for i in range(self.nsmp):
            ...
            ccdf_smps = lambda t: 1. - sl.cdf( (rm*gm(t)-sdm)/cm ) * sl.cdf( (rv*gv(t)-sdv)/cv )
            
            ## component Ml
            ##print "flexure"
            #ccdf_smps = lambda t: 1. - sl.cdf( (rm*gm(t)-sdm)/cm )
            
            ## component Vl
            ##print "shear"
            #ccdf_smps = lambda t: 1. - sl.cdf( (rv*gv(t)-sdv)/cv )
